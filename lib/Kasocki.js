...
class Kasocki {
    ...
    /**
     * Throws an Error if any topic in topics Array is not allowed
     * or if the topic does not currently exist.
     *
     * @param {Array} topics
    ...
    _checkTopicsAllowed(topics) {
        topics.map((topic) => {
            if (
                ...
                (this.allowedTopics && this.allowedTopics.indexOf(topic) < 0) ||
                // or if the topic does not currently exist
                (!this.kafkaConsumer._metadata.topics.find(e => e.name === topic))
            ) {
