...
const NotSubscibedError      = errors.NotSubscibedError;
...
class Kasocki {
    ...
    constructor(socket, options) {
        ...
        util.createKafkaConsumerAsync(kafkaConfig)
        ...
        .then(() => {
            this.availableTopics = util.getAvailableTopics(
                this.kafkaConsumer,
                options.allowedTopics
            );
            // Throw Error if there are no available topics.
    ...
    on_subscribe(topics) {
        ...
        // Else topics is a simple Array of topic names.
            ...
            topics = util.buildAssignments(this.kafkaConsumer, topics);
            this.log.info(
                {assignments: topics},
...
//     }
    ...
    on_consume() {
        ...
        if (!this.subscribed) {
            throw new NotSubscibedError('Cannot consume. Topics have not been subscribed or assigned.');
        }
        ...
        .then(util.buildMessageAsync)

        // Match it against any configured filters
        .then(m => util.matchAsync(m, this.filters))
        ...
        .catch((e) => {
            ...
            if ('origin' in e && e.origin === 'kafka') {
                ...
                switch (e.code) {
                    ...
                    case kafka.CODES.ERRORS.ERR__TIMED_OUT:
                        ...
                        break;
                    default:
                        this.log.error(e, 'Caught Kafka error while attempting to consume a matched message. Continuing');
                }
            }
            else {
            ...
            // keep looking.
            // TODO: we shouldn't just blindly call consume all the time.
            // We should be exlicit about what types of errors we will continue
            // with.  Anything else should throw and disconnect the client.
            return this.on_consume();
        });
    }
